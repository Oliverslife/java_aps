...
public class q16637 {
    static int n, ans = Integer.MIN_VALUE;      //최대값이 음수일 수 있잖아!
    ...
    static void dfs(int depth, int now) {
        ...
        //괄호 고려없이 순차진행 [1 > 2 순서]
        dfs(depth + 1, do_Op(now, nums.get(depth + 1), ops.get(depth)));    //지금 값과 다음 값을 연산

        //다음 연산을 미리 계산 후 현재 값과 연산 [2 > 1 순서]
        if(depth < ops.size() - 1) {
            int tmp = do_Op(nums.get(depth + 1), nums.get(depth + 2), ops.get(depth + 1));
